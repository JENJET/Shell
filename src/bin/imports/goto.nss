menu(type='*' where=window.is_taskbar||sel.count mode=mode.multiple title=title.go_to sep=sep.both image=\uE14A)
{
	menu(title='文件夹' image=\uE1F4)
	{
		...
		item(title='程序数据' image=inherit cmd=sys.programdata)
		item(title='所有程序' image=inherit cmd='shell:appsfolder')
		item(title='所有用户' image=inherit cmd=sys.users)
		separator
		...
		item(title='APP数据' image=inherit cmd=user.appdata)
		item(title='临时目录' image=inherit cmd=user.temp)
	}
	...
	menu(where=sys.ver.major >= 10 title=title.settings sep=sep.before image=\uE0F3)
	{
		...
		item(title='主机信息' image=inherit cmd-line='/K systeminfo')
		...
		{
			item(title='首页' image=inherit cmd='ms-settings:personalization')
		...
		{
			item(title='首页' image=inherit cmd='ms-settings:network-status')
			item(title='以太网' image=inherit cmd='ms-settings:network-ethernet')
			item(title='适配器' image=inherit cmd='shell:::{7007ACC7-3202-11D1-AAD2-00805FC1270E}')
		}
	}
}