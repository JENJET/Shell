menu(where=sel.count>0 type='file|dir|drive|namespace|back' mode="multiple" title='文件管理' image=\uE253)
{
	...
	{
		item(where=sel.count > 1 title='复制选中的 (@sel.count) 项' cmd=command.copy(sel(false, "\n")))
	...
	item(mode="single" type="file" title="修改后缀" image=\uE0B5 cmd=if(input("Change extension", "Type extension"),
	...
	item(type='file|dir|back.dir|drive' title='获取管理员所有权' image=[\uE194,#f00] admin
	...
	menu(type='file|dir|back.dir' mode="single" title='属性' image=icon.properties)
	{
		...
		item(title='归档' checked=io.attribute.archive(atrr)
			cmd args='/c ATTRIB @if(io.attribute.archive(atrr),"-","+")A "@sel.path"' window=hidden)
		separator
		item(title="创建时间" keys=io.dt.created(sel.path, 'y/m/d H:M') cmd=io.dt.created(sel.path,2000,1,1) vis=label)
		item(title="修改时间" keys=io.dt.modified(sel.path, 'y/m/d H:M') cmd=io.dt.modified(sel.path,2000,1,1) vis=label)
		item(title="访问时间" keys=io.dt.accessed(sel.path, 'y/m/d H:M') cmd=io.dt.accessed(sel.path,2000,1,1) vis=label)
	}

	menu(mode="single" type='file' find='.dll|.ocx' separator="before" title='注册服务' image=\uea86)
	{
		item(title='注册' admin cmd='regsvr32.exe' args='@sel.path.quote' invoke="multiple")
		item(title='卸载' admin cmd='regsvr32.exe' args='/u @sel.path.quote' invoke="multiple")
	}
	...
	menu(mode="single" type='back' expanded=true)
	{
		...
		{
			item(title='时间格式' cmd=io.dir.create(sys.datetime("ymdHMSs")))
